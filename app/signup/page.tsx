...
export default function SignupPage() {
    const [state, formAction, isPending] = useActionState(async (_prev: SignupState, formData: FormData) => {
        const result = await signup(formData);
        return result as SignupState;
    }, initialState);
            ...
            <div className="relative z-10 w-full max-w-sm px-4">
                <Card className="border-white/20 bg-white/80 shadow-2xl backdrop-blur-xl">
                    <CardHeader className="text-center pb-2">
                        <CardTitle className="text-3xl font-bold tracking-tight text-blue-950">Sign Up</CardTitle>
                    ...
                    <form action={formAction} className="flex flex-col gap-6">
                        <CardContent className="grid gap-4 pt-4">
                            ...
                            {state?.message && (
                                <p className="text-sm font-medium text-green-600 bg-green-50 p-2 rounded border border-green-100 text-center">{state.message}</p>
                            )}
                        </CardContent>
                        <CardFooter className="flex flex-col gap-4 pt-4">
                            ...
                            <div className="text-center text-sm text-blue-900/80">
                                ...
                                <Link href="/login" className="font-semibold text-blue-700 underline hover:text-blue-900">
                                    Login
                                </Link>
                            </div>
                        </CardFooter>
